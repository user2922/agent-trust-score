# Agent Trust Score — Specification

> **Canonical.** v1.1, validated 2026-08-28 by Prompt 1. Supersedes
> `SPEC_DRAFT.md`, which is kept only for provenance — when the two disagree,
> this file wins. Working title: **Agent Trust Score**; alt name **Blast
> Radius** (kept as the name of axis 2 either way).

## Fixed identifiers — stated once, referenced everywhere after
...
| Schema version | `1.0` |
| Checks | 37 — TS 7, BR 7, VF 8, CQ 8, OB 7 |

No later prompt restates these; they are imported or referenced.

## Resolved gaps — Prompt 1 spec validation
...
| Repo of 200,000 files | Silent partial answers are worse than refusals | Graded on what was analyzed, `truncated: true`, and every renderer states the truncation next to the grade. A truncated audit is never presented as complete |
| Repo with no commits | `git rev-parse HEAD` fails on an empty repo | `commit_sha` is `None`, OB-05 returns `not_applicable`, and the cache is bypassed — there is no SHA to key on |
| Cache keyed on commit SHA alone | A `--no-llm` report would be reused for a run that asked for enrichment | Key is `commit_sha` + `SCHEMA_VERSION`; a cached report with `llm.used == false` is a **miss** when the LLM is requested |
| What a report must never contain | The product ships reports to third parties | An unredacted secret; an absolute path outside the audited repo; a traceback; or any repo content beyond a 200-character redacted snippet |
| MCP tools accept local paths | A path argument is not a URL, and the schema said `url` | Both surfaces accept a URL or a local path; the argument is named `source`. The stdio server is local, so this grants no access the calling host lacks |

## Determinism — the stable payload

Rule D is asserted over `Report.stable_payload()`: the full report minus
`generated_at`, `run_ms`, and `LlmUsage`. Two audits of the same commit must
produce byte-identical stable payloads — with the LLM on and off, on any
machine. `report.json` as written to disk carries the volatile fields; the
determinism tests compare the stable payload, never the file bytes.
...
The `AXES` constant in `models.py` is the ordering authority for every axis
...
Encoded in `agent_trust/scoring/grades.py` and nowhere else. No other module in
the build may contain a band boundary.
...
- `--fix` — emits a draft `CLAUDE.md` and a `.gitignore` patch as unified diffs.
  **Not in v1**: the flag does not exist in the v1 CLI, and no build prompt
  implements it.
